...
OUTREACH_TEMPLATES: dict[str, str] = {
    ...
    "hiring_manager": (
        ...
        "{company} stood out for {business_problem}. My portfolio includes a career "
...
def generate_outreach(
    ...
    follow_up = FOLLOW_UP.format(title=job_title, top_skills=top_skills)
...
def generate_outreach_batch(jobs_df, contacts_df, scores: list[dict]) -> list[dict]:
    ...
    for _, contact in contacts_df.iterrows():
        ...
        for _, job in company_jobs.iterrows():
            if score_map.get(job["job_id"], {}).get("fit_score", 0) >= score_map.get(best_job["job_id"], {}).get("fit_score", 0):
                best_job = job
